...
const mocks = vi.hoisted(() => ({
  ...
  formatFullName: vi.fn((person: { name: string }) => person.name),
  getDateDisplayTitle: vi.fn((importantDate: { title: string }) => importantDate.title || 'Birthday'),
  getIntervalMs: vi.fn(),
}));
...
vi.mock('../../lib/i18n-utils', () => ({
  getTranslationsForLocale: (...args: unknown[]) => mocks.getTranslationsForLocale(...args),
}));

vi.mock('../../lib/nameUtils', () => ({
  formatFullName: (...args: unknown[]) => mocks.formatFullName(...args),
}));

vi.mock('../../lib/important-date-types', () => ({
  getDateDisplayTitle: (...args: unknown[]) => mocks.getDateDisplayTitle(...args),
}));

vi.mock('../../lib/upcoming-events', () => ({
  getIntervalMs: (...args: unknown[]) => mocks.getIntervalMs(...args),
}));
